...
void RestBeacon::onMessage(MessageCallback cb) { _messageCallback = cb; }

void RestBeacon::onDiscovery(DiscoveryCallback cb) { _discoveryCallback = cb; }
...
void RestBeacon::begin() {
  if (_messageCallback) {
    ...
    Serial.println(
        "[RestBeacon] HTTP server not started (no message callback set).");
  }
  ...
  if (_discoveryCallback) {
    ...
    Serial.println(
        "[RestBeacon] UDP listener not started (no discovery callback set).");
  }
}
...
void RestBeacon::loopHttp() { listenForMessage(); }

void RestBeacon::loopUdp() { listenForBroadcast(); }
