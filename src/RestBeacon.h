...
/**
 * @class RestBeacon
 * @brief Provides a RESTful interface and UDP-based discovery mechanism for
 * ESP32 devices.
 *
 * This class allows an ESP32 to listen for HTTP POST messages at a specific
 * "/message" endpoint and also notifies about UDP discovery broadcasts. It
 * enables both direct messaging and device discovery on a local network.
 */
class RestBeacon {
public:
  using MessageCallback = std::function<String(const Message &)>;
  using DiscoveryCallback =
      std::function<void(IPAddress, uint16_t, const String &)>;
   ...
   *
   * Call this method in loop() to continuously process HTTP and UDP
   * communication.
   */
  void loop();
   ...
   *
   * Call this method in loop() if you want to handle only HTTP communication
   * without UDP.
   */
  void loopHttp();
   ...
   *
   * Call this method in loop() if you want to handle only UDP-based discovery
   * without HTTP.
   */
  void loopUdp();

  /**
   * @brief Registers a callback to handle incoming HTTP POST requests on the
   * /message endpoint.
   * @param cb The callback to invoke when a valid message is received,
   * returning a string response to the request.
   */
  void onMessage(MessageCallback cb);

  /**
   * @brief Registers a callback to handle UDP discovery messages.
   * @param cb The callback to invoke when a message is received. Receives the
   * sender's IP address, port, and the message as parameters.
   */
